...
{	
    ...
    bool allProcessedMessage=true;
    // Execute sensor state machines
    for (uint8_t i = 0; i < numOfSensors; i++)
    {
        sensors[i]->loop();
        allProcessedMessage&=sensors[i]->hasProcessedMessage();
    }

    webConf->doLoop();
    
    if(connected && deepSleepInterval > 0 && publisher.isConnected() && allProcessedMessage && PANGO::TXQ.empty())
    {
